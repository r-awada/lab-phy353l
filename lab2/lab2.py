...
def LABLAB():
    ...
    def ploth(histo, title, binz, guess, label0):
        ...
        #norm = sum(hist[0])
        #y = [int(i)/norm for i in hist[0]] # JEREMY HELP
        #y0 = [i**(1/2) for i in y]
        #print(params, pcov)
        ...
        return params, pcov, entries

    def plotp(params, title, label0, entries):
    ...
    for num,i in enumerate(data):
        ...
        for j in range(1,2):
            ...
            i[f"params{j}"], i[f"pcov{j}"], i[f"entries{j}"] = ploth(i[j], i['title'], i[f'bins{j}'], i['guess'], f"Trial{j}")
            paired_data[num][f'bins{j}'] = np.arange(max(paired_data[num][j])) -0.5
            paired_data[num][f"params{j}"], paired_data[num][f"pcov{j}"], paired_data[num][f"entries{j}"] = ploth(paired_data[num][j], paired_data[num]['title'], paired_data[num][f'bins{j}'], paired_data[num]['guess'], f"Trial{j}")
        plt.show()
    for num, i in enumerate(data):
        for j in range(1,2):
            plotp(i[f"params{j}"], i['title'], f"Trial {j}", i[f"entries{j}"])
            print(i[f"pcov{j}"])
            plotp(paired_data[num][f"params{j}"], paired_data[num]['title'], f"Trial {j}", paired_data[num][f"entries{j}"])
            print(paired_data[num][f"pcov{j}"])
